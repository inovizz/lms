...
for (var i = 0; i <= allBooks.length-1; i++) {
	...
	if (parseInt(book[8]) < timestamp) {
		var borrower = '0x'+book[4].toString();
		var memberDetails = lms.getMemberDetailsByAccount(borrower);
		var borrowerEmail = memberDetails[2];
		var borrowerName = memberDetails[0];
		var issuedDate =  new Date(parseInt(book[8])*1000);
		// calculate duedate using issuedDate timestamp
		var dueDate = new Date(issuedDate.setDate(issuedDate.getDate()+10)).toLocaleString();
		// email payload
		var data = {"to": {[borrowerEmail] :borrowerName},
				...
				"html" : "<p>Hi "+titleCase(borrowerName)+",</p></br><p>\
						...
						- <b>'"+book[1]+"'</b> is overdue from the given due date - <b>"+dueDate+"</b>.</br> \
						Kindly return it to the respective Owner as per your \
						earliest convienence. In the case of any issues, \
						kindly contact Bookshelf admin at \
						<a href='mailto:admin@bookshelf.imaginea.com'>bookshelf@imaginea.com</a>.\
